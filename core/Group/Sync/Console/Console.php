...
class Console
{
    ...
    protected $options = [
        'generate:service' => [
            'args' => 'name',
            'command' => 'Group\Sync\Console\Command\GenerateServiceCommand',
            'help' => '生成一个自定义service'
        ],
        'generate:controller' => [
            'args' => 'name|groupname:name',
            'command' => 'Group\Sync\Console\Command\GenerateControllerCommand',
            'help' => '生成一个自定义controller(默认存放在src/Web,如果想要指定分组 groupname:name)'
        ],
        'sql:generate' => [
            'command' => 'Group\Sync\Console\Command\SqlGenerateCommand',
            'help' => '生成一个sql执行模板(存放于app/sql)'
        ],
        'sql:migrate' => [
            'command' => 'Group\Sync\Console\Command\SqlMigrateCommand',
            'help' => '执行sql更新'
        ],
        'sql:rollback' => [
            'args' => 'version',
            'command' => 'Group\Sync\Console\Command\SqlRollBackCommand',
            'help' => '执行sql回滚到指定版本'
        ],
    ];
